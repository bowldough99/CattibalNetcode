...
namespace StarterAssets
{
    ...
    public class PlayerMovement : NetworkBehaviour
    {
        ...
        public float AttackTimeout = 0.01f;
        ...
        [Tooltip("If the character is able to attack or not")]
        ...
        public override void OnNetworkSpawn()
        {
            ...
            if (IsClient && IsOwner)
            {
                //Grace come back here to redo the after 6 second cooldown timer to allow playerinput
        ...
        private void Move()
        {
            ...
            if (_input.move != Vector2.zero)
            {
                ...
                _targetRotation = Mathf.Atan2(0, inputDirection.y) * Mathf.Rad2Deg +
                                  _mainCamera.transform.eulerAngles.y;
        ...
        private void OnDeath(AnimationEvent animationEvent)
        {
            if (animationEvent.animatorClipInfo.weight > 0.5f)
            {
                AudioSource.PlayClipAtPoint(DeathAudioClip, transform.TransformPoint(_controller.center), FootstepAudioVolume);
            }
        }
        ...
        private void OnScratch(AnimationEvent animationEvent)
        {
            if (animationEvent.animatorClipInfo.weight > 0.5f)
            {
                AudioSource.PlayClipAtPoint(ScratchingAudioClip, transform.TransformPoint(_controller.center), FootstepAudioVolume);
            }
        }
